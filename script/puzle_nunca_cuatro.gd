...
# ¡CORREGIDO! Ruta al icono dentro del wrapper
@onready var icono_reloj = $LayoutPrincipal/PanelCapi/TimerWrapper/IconoReloj
...
@export var numero_de_nivel: int = 3
...
func _ready():
	...
	# --- SOLUCIÓN AL PIVOTE ---
	# Le decimos al reloj que gire sobre su propio centro
	# (Usamos call_deferred para asegurarnos de que ya tiene tamaño)
	icono_reloj.pivot_offset = icono_reloj.size / 2
	# --------------------------

	# 1. Ajusta la Cuadricula al nuevo tamaño
	cuadricula_node.columns = grid_size
	
	# 2. Inicializa el estado lógico
	...
	# 3. Genera la SOLUCIÓN COMPLETA
	generar_solucion()
	
	# 4. "Perforamos" la solución
	perforar_puzle()
...
func _on_timer_juego_timeout():
	...
	# --- GIRAR EL RELOJ ---
	icono_reloj.rotation_degrees += 90
...
# ... (perforar_puzle, generar_solucion, _es_movimiento_valido, etc.) ...

# Copia aquí el resto de funciones que ya tenías (perforar, generar, validar, UI, comprobar victoria...)
# Para ahorrar espacio no las repito, pero son idénticas al mensaje anterior.
# Asegúrate de incluir 'perforar_puzle', 'generar_solucion', '_es_movimiento_valido', 
# 'mostrar_mensaje', '_on_casilla_pressed', '_actualizar_textura', 
# '_on_boton_comprobar_pressed', '_on_boton_salir_pressed' y '_comprobar_victoria'.
			...
			if grid_state[r][c] == VACIO:
				...
				for tipo in opciones:
					...
					if _es_movimiento_valido(r, c):
						if generar_solucion(): return true
					grid_state[r][c] = VACIO
				return false
	return true
...
func _es_movimiento_valido(r, c):
	...
	if tipo_actual == VACIO: return true
	var contador = 0
	...
	for i in range(-grid_size + 1, grid_size):
		for j in range(grid_size):
			var r_diag = j; var c_diag = grid_size - 1 - (i + j)
			if c_diag >= 0 and c_diag < grid_size:
				if grid_state[r_diag][c_diag] == tipo_actual:
					contador += 1
					if contador >= 4: return false
...
func _on_boton_comprobar_pressed():
	...
	if es_valido == true:
		var mensaje_victoria = "¡Genial! Has dominado el arte de las secuencias. Los programadores usamos esta misma lógica para detectar errores y organizar la información sin que nada se repita o se rompa."
		print(mensaje_victoria)
		ProgresoJuego.completar_nivel(numero_de_nivel)
...
func _comprobar_victoria():
	for r in range(grid_size):
		for c in range(grid_size):
			if grid_state[r][c] == VACIO: return [false, "¡Oh! Parece que aún te quedan casillas vacías por rellenar."]
	for r in range(grid_size):
		var contador_circulos = 0; var contador_cruces = 0
		for c in range(grid_size):
			if grid_state[r][c] == CIRCULO: contador_circulos += 1; contador_cruces = 0
			elif grid_state[r][c] == CRUZ: contador_cruces += 1; contador_circulos = 0
			else: contador_circulos = 0; contador_cruces = 0
	...
	for c in range(grid_size):
		...
		for r in range(grid_size):
			...
			if contador_cruces >= 4: return [false, "¡Cuidado! Revisa la columna %s." % (c + 1)]
	for i in range(-grid_size + 1, grid_size):
		var contador_circulos_1 = 0; var contador_cruces_1 = 0
		var contador_circulos_2 = 0; var contador_cruces_2 = 0
		for j in range(grid_size):
			var r1 = j; var c1 = i + j; var r2 = j; var c2 = grid_size - 1 - (i + j)
			if c1 >= 0 and c1 < grid_size:
				if grid_state[r1][c1] == CIRCULO: contador_circulos_1 += 1; contador_cruces_1 = 0
				elif grid_state[r1][c1] == CRUZ: contador_cruces_1 += 1; contador_circulos_1 = 0
			...
			if c2 >= 0 and c2 < grid_size:
				if grid_state[r2][c2] == CIRCULO: contador_circulos_2 += 1; contador_cruces_2 = 0
				elif grid_state[r2][c2] == CRUZ: contador_cruces_2 += 1; contador_circulos_2 = 0
